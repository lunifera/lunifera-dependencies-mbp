 ...
 * 
 * Contributor:
 * 		Florian Pirchner - Fixed bug in findUserTransaction. Instanceof check used drools.TransactionManager
 * 		Florian Pirchner - Removes exception in debug-log-output
 */
...
public class JtaTransactionManager implements TransactionManager {
	...
	public static final String[] FALLBACK_TRANSACTION_MANAGER_NAMES = new String[] {
	...
	static {
		...
		try {
			transactionSynchronizationRegistryClass = cl
					.loadClass(TRANSACTION_SYNCHRONIZATION_REGISTRY_CLASS_NAME);
	...
	public JtaTransactionManager(Object ut, Object tsr, Object tm) {
		if (ut instanceof UserTransaction) {
			...
			this.ut = (UserTransaction) ((ut != null) ? ut
					: findUserTransaction());
		}
		...
		if (tm instanceof javax.transaction.TransactionManager) {
			...
			this.tm = (javax.transaction.TransactionManager) ((tm != null) ? tm
					: findTransactionManager(this.ut));
		}
		this.tsr = (tsr != null) ? tsr
				: findTransactionSynchronizationRegistry(this.ut, this.tm);
	}

	protected javax.transaction.TransactionManager findTransactionManager(
			UserTransaction ut) {
		if (ut instanceof javax.transaction.TransactionManager) {
			logger.debug(
					"JTA UserTransaction object [{}] implements TransactionManager",
					ut);
	...
	protected UserTransaction findUserTransaction() {
		try {
			...
			return (UserTransaction) context
					.lookup(DEFAULT_USER_TRANSACTION_NAME);
		} catch (NamingException ex) {
			logger.debug("No UserTransaction found at JNDI location [{}]",
					DEFAULT_USER_TRANSACTION_NAME, ex);
	...
	public void registerTransactionSynchronization(
			final TransactionSynchronization ts) {
		if (this.tsr != null) {
			TransactionSynchronizationRegistryHelper
					.registerTransactionSynchronization(this.tsr, ts);
		} else if (this.tm != null) {
			try {
				this.tm.getTransaction().registerSynchronization(
						new JtaTransactionSynchronizationAdapter(ts));
			} catch (Exception e) {
