...
/**
 * This is the object that contains the marshalled byte stream of information
 * representing the ProcessInstance class.
 * 
 * Because of Hibernate 3.3.x/3.4.x <-> 4.x compatibility issues, the mapping
 * for this class has been moved to
 */
@Entity
public class ProcessInstanceInfo {
	 ...
	 * <ul>
	 * <li>(field access is inefficient/dangerous, and not necessary)</li>
	 * </ul>
	 * </li>
	 ...
	 * </p> So instead of that, we just add the getters and use
	...
	public ProcessInstance getProcessInstance(
			InternalKnowledgeRuntime kruntime, Environment env) {
		this.env = env;
		if (processInstance == null) {
			try {
				...
				MarshallerReaderContext context = new MarshallerReaderContext(
						...
						ProtobufMarshaller.TIMER_READERS, this.env);
	 ...
	 * <li>..which actually (re)sets all variables in
	 * {@link VariableScopeInstance}.setContextInstanceContainer(...)</li>
	 * <li>This of course causes {@link ProcessEventSupport}
	 * .fireBeforeVariableChanged(...) to fire.</li>
	 * <li>Then the {@link org.jbpm.process.audit.JPAWorkingMemoryDbLogger} ends
	 * up logging a variable change -- but the associated process instance
	 * hasn't been persisted yet.</li>
	 * <li>So the variable instance change is associated with process instance
	 * "0"</li>
	 * <li>...and can never be retrieved, because "0" is not a valid id.</li>
	 * </ul>
	 * </p> Normally, the variable change is logged after the following method
	 * has completed.
	 */
	@PreUpdate
	@PrePersist
	public void update() {
		...
		try {
			...
			Object result = marshaller.writeProcessInstance(context,
					processInstance);
			if (marshaller instanceof ProtobufRuleFlowProcessInstanceMarshaller
					&& result != null) {
